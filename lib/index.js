...
class ErrsolePostgres extends EventEmitter {
  ...
  async postLogs (logEntries) {
    ...
    const query = 'INSERT INTO errsole_logs (hostname, pid, source, level, message, meta) VALUES ($1, $2, $3, $4, $5, $6)';
        ...
        deletedRowCount = await new Promise((resolve, reject) => {
          this.pool.query(
            ...
            (err, result) => {
              ...
              if (!result) {
                console.error('Unexpected result format while deleting logs from errsole_logs');
