...
class ErrsolePostgres extends EventEmitter {
  ...
  async createTables () {
    const queries = [
      ...
      `CREATE INDEX IF NOT EXISTS ${this.logsTable}_source_level_id_idx ON ${this.logsTable} (source, level, id)`,
      `CREATE INDEX IF NOT EXISTS ${this.logsTable}_source_level_timestamp_idx ON ${this.logsTable} (source, level, timestamp)`,
      `CREATE INDEX IF NOT EXISTS ${this.logsTable}_hostname_pid_id_idx ON ${this.logsTable} (hostname, pid, id)`,
      `CREATE INDEX IF NOT EXISTS idx_errsole_id ON ${this.logsTable} (errsole_id)`,
      ...
      `CREATE INDEX IF NOT EXISTS idx_${this.notificationsTable}_host_hashed_created ON ${this.notificationsTable} (hostname, hashed_message, created_at)`,
      `CREATE INDEX IF NOT EXISTS idx_${this.notificationsTable}_created_at ON ${this.notificationsTable} (created_at)`,

     `ALTER TABLE ${this.configTable} ALTER COLUMN value TYPE TEXT;`
    ];
  ...
  async getLogs (filters = {}) {
    ...
    const query = `SELECT id, hostname, pid, source, timestamp, level, message, errsole_id FROM ${this.logsTable} ${whereClause} ORDER BY id ${sortOrder} LIMIT $${values.length + 1}`;
  ...
  async searchLogs (searchTerms, filters = {}) {
    ...
    const whereClause = whereClauses.length ? `WHERE ${whereClauses.join(' AND ')}` : '';
    const query = `SELECT id, hostname, pid, source, timestamp, level, message,errsole_id FROM ${this.logsTable} ${whereClause} ORDER BY id ${sortOrder} LIMIT $${index}`;
    values.push(filters.limit);
  ...
  /**
   * Clears all the logs in the logs table.
   *
   * @async
   * @function DeleteAllLogs
   * @returns {Promise<void>} - A promise that resolves when all logs have been deleted.
   * @throws {Error} - Throws an error if the operation fails.
   */
  async DeleteAllLogs () {
