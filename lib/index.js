...
class ErrsolePostgres extends EventEmitter {
  ...
  async insertNotificationItem (notification = {}) {
    ...
    const client = await this.pool.connect(); // Acquiring a PostgreSQL client from the pool
    ...
    try {
      ...
      return {
        previousNotificationItem: previousNotificationItem || null,
  ...
  async deleteExpiredNotificationItems () {
    ...
    try {
      ...
      const expirationTime = new Date(Date.now() - notificationsTTL).toISOString(); // ISO format works fine for PostgreSQL
      let deletedRowCount;

      do {
        // Use a common table expression (CTE) to delete in batches of 1000 rows
        const deleteQuery = `
          WITH deleted AS (
        ...
        // Wait for 10 seconds before running the next batch
